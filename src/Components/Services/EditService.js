...
const EditService=()=>{
    ...
    useEffect(()=>{
        if(Object.keys(editservice).length === 0 && editservice.constructor === Object){
            geteditData(localStorage.getItem("EditServiceId"))
        }
    ...
    return(
        <>
            ...
            <Card style={{ width: '100%' }}>
                <Form initialValues={{...editservice}} ref={ServiceForm} name="nest-messages" onFinish={onEditFinish} validateMessages={validateMessages}>
                    <Form.Item name={[ 'name']} label="Name" rules={[{ required: true }]}>
                        <Input placeholder="Enter name" />
                    </Form.Item>
                    <Form.Item name={['type_id']} label={"Type"}>
                        <TypeSelect id={editservice.type_id} types={types} setItems={setTypes} SelectChange={SetTypeChange} />
                    </Form.Item>
                    <Form.Item name={[ 'cat']} label="Category">
                        <CategorySelect id={editservice.category_id} items={items} setItems={setItems} SelectChange={SelectChange} />
                    </Form.Item>
                ...
                <Button.Group style={{float:'right',marginTop:'20px'}}>
                    ...
                    <Button type="primary" onClick={handleOk} style={{margin:'5px'}} >Update</Button>
                </Button.Group>
                </div>
            </Card>
        </>
